...
(`.kb-tablist`, `.kb-tab`, `.kb-tab-panel` — built via `makeTabs()`, see
...
| Tabs        | `makeTabs(items, opts)` — `items: { label, content: Node }[]`, `opts.defaultIndex` (default `0`); returns the built `.kb-tablist` + panels, wired to switch on click |
